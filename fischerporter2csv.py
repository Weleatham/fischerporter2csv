...
workdir = '/home/ec2-user/environment/Projects/Working/fischer-to-csv/'
defcsv = 'Calculator_2019.csv'
...
# identifier for the station.
            ...
            inwf = row[8]
            emal = row[9]
            stationlist.append([snum,steid+numid,stid,sloc,stvl,edvl,totl,full,
            qual,inwf,emal])
        return stationlist
...
# Here I change the index of the dataframe so it is a timestamp. 
...
def main():
    ...
    header = ['Station Number','Identifier','STID','Location','SRG Value',
    'End Value','Total','%Full','Quality','IN WFO','Email Address']
    statsdf = pd.DataFrame(stationidentifier(defcsv), columns = header)
    data = fischerdata(workdir)
...
# Sorting the dataframe by the %Full then writing it to a file
    ...
    finaldf.to_csv(defcsv,index=False)
    print finaldf
